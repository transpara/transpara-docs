...
* **OPERATION\_MODE:** Defines if the service is active. Options: `STOP`, `RUN`. (Default: `STOP`)  
* **LOG\_ONLY:** Boolean flag. If true, data is logged to the terminal but not sent to tStore. (Default: `true`)  
* **SOURCE\_TZ:** Source timezone name (IANA), used for timestamp handling. (Default: `UTC`)  
* **LOG\_LEVEL:** Logging verbosity (e.g., TDEBUG, INFO).

### tStore Output (Required for RUN \+ LOG\_ONLY: False)

* **TSTORE\_API\_URL:** Base URL for the tstore-interface API.  
* **TSTORE\_FLUSH\_INTERVAL\_SECONDS:** How often to flush queued data (seconds).  
* **DATASET\_NAME:** Metric/dataset name used when writing to tStore. (Default: `OPCUA_EXTRACTOR`)

### OPC UA Connection

* **opcua\_endpoint\_url:** The target URL (e.g., `opc.tcp://10.0.0.10:4840`).  
* **security\_mode:** None, Sign, or SignAndEncrypt.  
* **security\_policy:** e.g., Basic256Sha256, Aes128Sha256RsaOaep.  
* **auth\_method:** Anonymous, UserName, or Certificate.

### Monitoring and Subscription

* **sampling\_interval:** Default sampling frequency in milliseconds. (Default: `200`)  
* **publishing\_interval:** Subscription publishing interval in milliseconds. (Default: `250`)  
* **timestamp\_source:** Which timestamp to use: `gather` (client time), `server`, or `source`.
...
List of monitored items (in YAML format):
