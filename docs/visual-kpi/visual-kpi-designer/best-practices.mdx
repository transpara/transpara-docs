...
slug: /visual-kpi/visual-kpi-designer/best-practices
...
Language settings are sitewide. You set the language in the Visual KPI Designer, and everyone who accesses that instance of Visual KPI will see the site with that language setting. You should also note that changing the language settings will only change the site's language, not your data's. For example, if you have a group of KPIs called "Management", and you change the site's language to Spanish, that group will still be called "Management". You can read more on the [Language guide](/visual-kpi/visual-kpi-designer/settings/advanced#language).
