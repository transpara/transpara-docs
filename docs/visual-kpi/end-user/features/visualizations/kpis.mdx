---
title: KPIs
slug: /visual-kpi/end-user/features/visualizations/kpis
...
1. **[Filter](/visual-kpi/end-user/basic-navigation/visualization/#filters)** (<SvgFont icon="184"/>): Filter by status or distance from the location where you are.
...
3. **[Only Show Descendant/Child Groups](/visual-kpi/end-user/basic-navigation/visualization/#only-show-descendantchild-groups)** (<SvgFont icon="208"/>/<SvgFont icon="207"/>): Show all child items of the Group you are currently viewing or collapse the list to go to the top level of the current list.
...
7. **[View as List](/visual-kpi/end-user/basic-navigation/visualization/#group-roll-upslist-view)** (<SvgFont icon="206"/>): This is the default view in the Group view. If you changed the view, click this icon to return to a list view.
...
12. **[View as KPI Map](/visual-kpi/end-user/basic-navigation/visualization/#view-as-kpi-map)** (<SvgFont icon="203"/>): KPI maps, if configured by your admin, show a heat map of all the KPIs in the level you are currently viewing.
13. **[View as Group Map](/visual-kpi/end-user/basic-navigation/visualization/#view-as-group-map)** (<SvgFont icon="141"/>): Shows a bar across the top of each Group to indicate the status of the Group as determined by the Group Map settings.
14. **[View as Group Map Table](/visual-kpi/end-user/basic-navigation/visualization/#view-as-group-map-table)** (<SvgFont icon="21"/>): Similar to the Group Map View, it shows the groups as titles in a table.
15. **[View as Geo Table](/visual-kpi/end-user/basic-navigation/visualization/#view-as-geo-map)** (<SvgFont icon="189"/>): The Group Map view is similar to the KPI Map but gives each Group its status.  
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/kpis-top-bar.png"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/group-by-status.gif"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/multi-select-kpi-charts.gif"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/bulletchart-detail.webp"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/bullet-chart.png"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/status-history-chart.png"/>

<Image src="media/visual-kpi/end-user/features/visualizations/kpis/status-history-chart-background.png"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/status-view-summary.png"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/status-history.png"/>
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/notification.png"/>

## Optional KPI insights

Visual KPI provides detailed context to help you make sense of complex metrics. With features like Influencer KPIs, map views, links, and detail pages, you can quickly uncover what's driving performance, where it is happening, and how it connects to broader operations.

### View influencer KPIs

The Influencer KPI feature provides visibility into the KPIs that significantly affect a parent KPI's performance. This allows users to better understand the driving factors behind changes in complex metrics, such as fuel cost or average temperature. For example, if a top-level KPI such as “Fuel Cost” is behaving unexpectedly, the Influencer KPI feature helps identify which underlying KPIs most affect it—regardless of whether they are direct children in the KPI hierarchy.

The KPI detail view includes a new icon (<PngFont name='influencers22' />) that lets users switch from the standard hierarchical drill-down to an influencer view, which highlights KPIs that have a direct impact on the selected metric.

<Image src="media\end-user\features\visualizations\kpis\influencer-kpis.png"/>

### View links

You'll find links all over Visual KPI. Links can be configured by Visual KPI administrators or users. The link view isn't limited to internal links. You can add external links to software or other tools you often use. Some of the most common places you'll find the link icon (<SvgFont icon="261"/>) are:
...
2. **Dashboard link widgets**: using the [Customize Dashboard widget](/visual-kpi/end-user/features/dashboards), you can tie a link to a Dashboard. However, only profiles with access to these Dashboards will see them.
3. **Bookmark bar links**: Links on the [Bookmark bar](/visual-kpi/end-user/features/profiles) are tied to a profile.
...
If you click the View on map icon (<SvgFont icon="264"/>), you can view the geographical location of KPIs with associated latitude and longitude data. When this information is provided, users can visualize KPI performance in a spatial context, supporting more informed, location-based analysis. KPI coordinates must be added by the Designer during setup. If coordinates are missing, the KPI will not appear on the map.
...
<Image src="media/visual-kpi/end-user/features/visualizations/kpis/kpi-details.gif"/>

## What's next?

Learn the best way to handle [Charts](/visual-kpi/end-user/features/visualizations/charts) on our page and get the best out of Visual KPI.