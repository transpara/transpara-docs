...
2. **[Grouping](#grouping)** (<SvgFont icon="114"/>): Allows you to group your KPIs by Standard or Custom Attributes such as Status, Group, or any other that makes sense for your organization. Your admin can configure Custom Attributes.
...
6. **[Select KPIs](#select-kpis)** (<SvgFont icon="255"/>): Select multiple KPIs by clicking the boxes that appear to the left of the KPIs in the list.
...
8. **[View as Bullet Chart](#view-as-bullet-chart)** (<SvgFont icon="171"/>): It is similar to the KPI List View and Status History view, but the KPI Bullet Chart view focuses on the current status of the KPI relative to targets and limits.
9. **[View as Trend List](#view-as-trend-list)** (<SvgFont icon="192"/>): Present the KPIs as trends so you can quickly compare the KPIs values from your Group.
10. **[View as Status History Chart](#view-as-status-history-chart)** (<SvgFont icon="37"/>): Projects vertical color bars to show Highs and Lows or other targets based on status color.
11. **[View as Status History Map](##view-as-status-history-map)** (<SvgFont icon="37" rotate/>): Similar to Status History Chart, but you can see a status history chart in the trend view. Click the Status Background icon to activate the background.
...
14. **[View as Group Map Table](#view-as-group-map-table)** (<SvgFont icon="21"/>): Similar to the Group Map View, it shows the groups as titles in a table.
...
16. **[View Status Summary](#view-as-status-summary)** (<SvgFont icon="136"/>): This shows you a summary and history of your best and worst KPIs at the group level in Pareto chart form.
17. **[View Status History](#view-status-history)** (<SvgFont icon="138"/>): This shows you the relative performance of the selected KPIs grouped into the various status options.
18. **[View Notification History](#view-notification-history)** (<SvgFont icon="137"/>): See how many times the system sent notifications for alert status KPIs.
...
## View as Status History Map

The **Status History Map** visualization (<SvgFont icon="37" rotate/>) highlights how a KPI's state evolves over time by showing discrete status changes like transitions between "Good," "High," and "Low." Unlike traditional charts, it focuses on when and how often status changes occur, making it easy to identify erratic or inconsistent behavior at a glance.

The system records KPI status history automatically, capturing each transition as color-coded segments. By default, the map starts recording when a KPI is created, excluding prior data unless the Designer backfills it. This ensures the visualization reflects accurate, real-time changes.

This feature is ideal for tracking events where status changes matter more than trends, such as machine start/stop cycles or states like "On," "Off," or "Standby." It provides a clear view of when these states occur, making it perfect for large-screen dashboards and quick performance monitoring.

<Image src=""/>

## View Status Summary

Visual KPI shows a summary and history of your best and worst KPIs at the group level. Status Summary (<SvgFont icon="136"/>) allows you to track over time the history of KPIs in alert. You can see the status of the entire group as a Pareto chart and then get more details on individual KPIs. You can also adjust the time range to see a summary over a given reference and freely scroll backward and forward in time.
...
You can switch between alert count and time-in-state. Click on the Status History icon (<SvgFont icon="138"/>), then select the **Alert Count icon** (<SvgFont icon="151"/>) to see how many times a KPI has reached a particular status over time. If you select the **Time-in-State icon** (<SvgFont icon="287"/>), you'll see how much time a KPI spent in each status.
